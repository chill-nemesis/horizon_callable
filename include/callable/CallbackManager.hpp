...
namespace HORIZON::CALLABLE
{
	...
	namespace INTERNAL
	{
		...
		protected:
			 ...
			 * @warning If you pass the handle, you must move it. If you create a copy, the handle is destroyed and
			 * unregisters itself!
			 * @return  The handle
			 */
	 ...
	 * After creating a callback, a handle will be returned. As long as the handle exists, the callback is guaranteed to
	 * exist. A callback handle keeps a reference to its corresponding callback manager. If a registered handle is
	 * destroyed, it will self-unregister from the callback manager. Because of this behaviour, a CallbackHandle can
	 * only be passed by move operations.
	 *
	 * Using an object for managing the lifetime of callbacks allows unregistering said callbacks once the associated
	 * object is destroyed.
	 */
	struct CallbackHandle
	{
	...
	private:
		 ...
		 */
		explicit CallbackHandle(Identifier const& id, Manager manager) : _id(id), _manager(std::move(manager)) { }

	public:
		// make sure that there cannot be any copies of this object
		HORIZON_NoCopy(CallbackHandle)
		// enable move operations
		HORIZON_DefaultMove(CallbackHandle)

		/*!
		 * @return true iff the handle references a valid callback at a manager.
		 */
		[[nodiscard]] inline bool IsRegistered() const noexcept { return (bool)_manager; }

		/*!
		 * @return The identifier of the handle.
		 */
		[[nodiscard]] inline Identifier const& GetIdentifier() const noexcept { return _id; }
	...
	namespace INTERNAL
	{
		...
		protected:
			struct CallbackEntry
			{
				...
				// TODO: is this POD?
				CallbackEntry(Callback&& callback, Identifier const& id) : _callback(callback), _id(id) { }

				bool operator==(CallbackEntry const& other) noexcept { return _id == other._id; }

				bool operator==(Identifier const& other) noexcept { return _id == other; }
		...
		public:
			...
			CallbackHandle Register(Callback&& callback)
			{
				...
				// NOTE: do not use std::move here because of copy elision
			...
			{
				for (auto const& cbf : _callbacks) { cbf(args...); }
			}
	...
	public:
		...
		std::vector<Result> operator()(Args&&... args) const
		{
			...
			int i = 0;
			for (auto& cbf : this->_callbacks) { results[i++] = cbf(args...); }
		 ...
		 */
		std::vector<Result> Call(Args&&... args) const { return this->operator()(std::forward<Args>(args)...); }
	...
	public:
		 ...
		 * @note There is no guarantee about the call-order.
		 */
		inline constexpr void Call(Args&&... args) const { return this->CallNoResult(std::forward<Args>(args)...); }
